...
import { config, sysConfig } from "./config.ts"
...
async function convertAllMarkdown() {
  ...
  if (debug) {
    ...
    const outputDir = (
      await fs.promises.readdir(path.join(__dirname, "../output"))
    ).filter((file) => file.endsWith(".pdf"))
    if (outputDir.length !== 0) {
      console.warn(chalk.yellow("当前输出目录不为空"))
      if (sysConfig.autoClean) {
        console.info(chalk.yellowBright("尝试清空输出目录中的pdf文件"))
        outputDir.forEach((pdf) => {
          try {
            fs.unlinkSync(path.join(__dirname, "../output", pdf))
          } catch (error) {
            console.error(chalk.red(`删除${pdf}失败`))
          }
          console.info(chalk.green(`删除${pdf}成功`))
        })
      }
    }
    ...
    const mdFiles = files.filter(
      (file) => file.endsWith(".md") && file !== "test.md",
    )
    if (mdFiles.length === 0) {
    ...
    console.log(
      chalk.green(`找到 ${mdFiles.length} 个 Markdown 文件:`, mdFiles),
    )

    for (const file of mdFiles) {
      const outputFileName = file.replace(".md", ".pdf")
      let outputPath = newConfig.dest?.replace(
        "<--file_name-->",
        outputFileName,
      )
      newConfig.dest = path.join(
        __dirname,
